...
-- Total purchases validation
    ...
    FROM bronze.customer_changes_stream   --Pulling incremental from the stream
    WHERE  customer_id IS NOT NULL AND email is not null     -- Basic data quality rule
    ) AS source  
     
--Merge operation will happen based on this column
...
-- Create task to call stored proc.
